...
print("Initial Centroids (Random Positions)  : ")
print(C)
print( C.shape )    # (3,2)
...
while error.all():       # error != 0
    ...
    for i in range(len(X)):        # len(X) : 3000
        ...
        clusters[i] = cluster
    # Storing the old centriod values
    C_old = deepcopy(C)
    # Finding the new centriods by taking the average value
    for s in range(k):   # k=3  because we have to find 3 centroid location
